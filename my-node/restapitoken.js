...
const jwtObject = {
    'jwtSecret': 'xyzprq00700qrpzyx',     // for access token
    "newSecret":  "kdkngrtg664ha079"      // for refresh token
}
...
//sequelize connect to mysql database
        ...
        .then((result) => {
            ...
            if (result === null) {
                ...
                if (result.Password !== authValue.Password) {
                    ...
                    // 2b. Logic for issuing the refresh token
                    ...
                    // 2c. respond refresh token to client
                    response.send({
                        statusCode: 200,
                        authenticated: true,
                        data: accessToken
...
// verify token when user send toke in HTTP request
instance.set('newSecret', jwtObject.newSecret);
// 2. Authorize the user and generate token
instance.post('/token', (request, response) => {
    const authValue = {
        EmailId: request.body.EmailId,
        Password: request.body.Password
    };
        ...
        .then((result) => {
            ...
            if (result === null) {
                ...
                if (result.Password !== authValue.Password) {
                    ...
                    let RefreshToken = jwtoken.sign(result.toJSON(), instance.get('newSecret'), {
                        expiresIn: 30 // token will expire in 3600 seconds
                    });
                    globalTokan = RefreshToken;
